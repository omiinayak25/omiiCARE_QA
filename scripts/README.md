...
Cross-platform setup/start/stop/reset/health-check scripts (.sh and .bat) so a new developer can clone → setup → start → develop with minimal manual steps.

## Planned Contents

- `setup`, `start`, `stop`, `reset`, `health-check` (`.sh` + `.bat`)
