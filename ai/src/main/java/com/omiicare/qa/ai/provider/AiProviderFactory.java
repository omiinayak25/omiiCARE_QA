...
/**
 * Resolves the configured {@link AiProvider} (Factory pattern). Selection is configuration-driven
 * and vendor-neutral: {@code local} for offline work, or a hosted provider (Claude / OpenAI) that
...
public final class AiProviderFactory {
    ...
    public static AiProvider create(AiConfig config) {
        String provider =
                config.provider() == null ? "local" : config.provider().toLowerCase(Locale.ROOT);
